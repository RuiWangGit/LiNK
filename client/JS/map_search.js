                ...
                for (var i=0; i<teamlabels.length; i++)
                  {
                    for (var j = 0; j < input.length; j++)
                      {
                        characters = characters + teamlabels[i][j];
                      }
                    output.push(characters);
                    if(input == output[i])
                      {
                        var marker = new google.maps.Marker({
                        position: new google.maps.LatLng(coordinates[2*i], coordinates[2*i + 1]),
                        map: map,
                        title: teamlabels[i]
                        });
                        var set = true;
